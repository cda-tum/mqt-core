...
#include "CatalystQuantum/QuantumOps.h"
...
#include "CatalystQuantum/QuantumEnums.cpp.inc"
#define GET_OP_CLASSES
#include "CatalystQuantum/QuantumOps.cpp.inc"
...
static const mlir::StringSet<> hermitianOps = {"Hadamard", "PauliX", "PauliY",
                                               "PauliZ",   "CNOT",   "CY",
                                               "CZ",       "SWAP",   "Toffoli"};
static const mlir::StringSet<> rotationsOps = {
    "RX",  "RY",  "RZ",  "PhaseShift",
    "CRX", "CRY", "CRZ", "ControlledPhaseShift"};

LogicalResult StaticCustomOp::canonicalize(StaticCustomOp op,
  ...
  if (rotationsOps.contains(name)) {
    ...
    rewriter.replaceOpWithNewOp<StaticCustomOp>(
        ...
        rewriter.getDenseF64ArrayAttr(paramsNeg), op.getInQubits(), name,
...
LogicalResult CustomOp::canonicalize(CustomOp op,
                                     mlir::PatternRewriter& rewriter) {
  if (!op.getAdjoint()) {
    return failure();
  }
  ...
  if (rotationsOps.contains(name)) {
    ...
    rewriter.replaceOpWithNewOp<CustomOp>(
        op, op.getOutQubits().getTypes(), op.getOutCtrlQubits().getTypes(),
        paramsNeg, op.getInQubits(), name, nullptr, op.getInCtrlQubits(),
        op.getInCtrlValues());
...
LogicalResult MultiRZOp::canonicalize(MultiRZOp op,
                                      mlir::PatternRewriter& rewriter) {
  if (op.getAdjoint()) {
    auto paramNeg =
        rewriter.create<mlir::arith::NegFOp>(op.getLoc(), op.getTheta());

    rewriter.replaceOpWithNewOp<MultiRZOp>(
        op, op.getOutQubits().getTypes(), op.getOutCtrlQubits().getTypes(),
        paramNeg, op.getInQubits(), nullptr, op.getInCtrlQubits(),
        op.getInCtrlValues());
...
static LogicalResult verifyObservable(Value obs,
...
LogicalResult QubitUnitaryOp::verify() {
  ...
  if (failed(verifyTensorResult(cast<ShapedType>(getMatrix().getType()), dim,
                                dim))) {
    return emitOpError(
        "The Unitary matrix must be of size 2^(num_qubits) * 2^(num_qubits)");
  }
...
LogicalResult HermitianOp::verify() {
  ...
  if (failed(verifyTensorResult(cast<ShapedType>(getMatrix().getType()), dim,
                                dim)))
    return emitOpError(
        "The Hermitian matrix must be of size 2^(num_qubits) * 2^(num_qubits)");
...
LogicalResult CountsOp::verify() {
  ...
  if (getObs().getDefiningOp<NamedObsOp>()) {
    ...
    // In the computational basis, the "eigenvalues" are all possible bistrings
    // one can measure.
    numEigvals = std::pow(2, numQubits.value());
  } else {
    return emitOpError(
        "cannot determine the number of eigenvalues for general observable");
  }
...
LogicalResult AdjointOp::verify() {
  auto res = this->getRegion().walk(
      [](MeasurementProcess op) { return WalkResult::interrupt(); });
