...
struct FromQuantumComputationPattern final : OpRewritePattern<AllocOp> {
  ...
  ExtractOp createRegisterAccess(AllocOp& reg, size_t index,
    ...
    return rewriter.create<ExtractOp>(
        reg.getLoc(), QubitType::get(rewriter.getContext()), reg.getResult(),
        nullptr, rewriter.getI64IntegerAttr(index));
  }

  CustomOp createCustomOp(std::string name, mlir::Location loc,
                          mlir::ValueRange inputs,
                          mlir::ValueRange controlQubits,
                          mlir::ValueRange controlValues,
                          PatternRewriter& rewriter) const {
    return rewriter.create<CustomOp>(loc, name, inputs, controlQubits,
                                     controlValues);
  }
  ...
  void rewrite(AllocOp op, PatternRewriter& rewriter) const override {
    ...
    for (size_t i = 0; i < numQubits; i++) {
      currentQubitVariables[i] =
          createRegisterAccess(newAlloc, i, rewriter).getResult();
    }
    ...
    for (const auto& o : circuit) {
      ...
      case qc::OpType::Y:
      case qc::OpType::Z:
        newOp = createCustomOp(opName + opNames[o->getType()], op->getLoc(),
                               {currentQubitVariables[o->getTargets()[0]]},
                               controlQubits, controlValues,
