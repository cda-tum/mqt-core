...
TEST_F(IO, qiskitMcxGray) {
  std::stringstream ss{};
  ss << "qreg q[4];"
     << "mcx_gray q[0], q[1], q[2], q[3];" << std::endl;
...
TEST_F(IO, qiskitMcphase) {
  std::stringstream ss{};
  ss << "qreg q[4];"
     << "mcphase(pi) q[0], q[1], q[2], q[3];" << std::endl;
  qc->import(ss, qc::Format::OpenQASM);
  auto& gate = *(qc->begin());
  std::cout << *qc << std::endl;
...
TEST_F(IO, qiskitMcxVchain) {
  ...
  ss << "qreg q[4];"
     << "qreg anc[1];"
     << "mcx_vchain q[0], q[1], q[2], q[3], anc[0];" << std::endl;
  qc->import(ss, qc::Format::OpenQASM);
  auto& gate = *(qc->begin());
  std::cout << *qc << std::endl;
  EXPECT_EQ(gate->getType(), qc::X);
...
TEST_F(IO, qiskitMcxRecursiveInDeclaration) {
  ...
  ss << "qreg q[7];"
     ...
     << "bar q[0], q[1], q[2], q[3], q[4], q[5], q[6];" << std::endl;
  qc->import(ss, qc::Format::OpenQASM);
  std::cout << *qc << std::endl;
...
TEST_F(IO, qiskitMcxVchainInDeclaration) {
  ...
  ss << "qreg q[5];"
     ...
     << "foo q[0], q[1], q[2], q[3], q[4];" << std::endl;
  qc->import(ss, qc::Format::OpenQASM);
  std::cout << *qc << std::endl;
  const auto& op = qc->at(0);
  EXPECT_EQ(op->getType(), qc::X);
...
TEST_F(IO, qiskitMcxDuplicateQubit) {
  ...
  ss << "qreg q[4];"
     << "qreg anc[1];"
     << "mcx_vchain q[0], q[0], q[2], q[3], anc[0];" << std::endl;
  EXPECT_THROW(qc->import(ss, qc::Format::OpenQASM), std::runtime_error);
}
...
TEST_F(IO, qiskitMcxQubitRegister) {
  ...
  ss << "qreg q[4];"
     << "qreg anc[1];"
     << "mcx_vchain q, q[0], q[2], q[3], anc[0];" << std::endl;
  EXPECT_THROW(qc->import(ss, qc::Format::OpenQASM), std::runtime_error);
}
