...
TEST(NAOperation, ShuttlingOperation) {
  ...
  EXPECT_ANY_THROW(NAShuttlingOperation(
      ShuttleType::STORE, std::vector{std::make_shared<Location>(0, 0)},
      std::vector{std::make_shared<Location>(0, 1),
                  std::make_shared<Location>(1, 1)}));
}
...
TEST(NAOperation, LocalOperation) {
  ...
  EXPECT_ANY_THROW(
      NALocalOperation(FullOpType{qc::ECR, 0}, std::make_shared<Location>(0, 0)));
  EXPECT_ANY_THROW(
      NALocalOperation(FullOpType{qc::RY, 1}, std::make_shared<Location>(0, 0)));
}
