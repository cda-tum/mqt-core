...
namespace zx {
    ...
    bool checkSpiderFusion(const ZXDiagram& diag, const Vertex v0, const Vertex v1) {
    ...
    void localComp(ZXDiagram& diag, const Vertex v) { // TODO:scalars
        ...
        diag.addGlobalPhase(PiExpression(PiRational{diag.phase(v).getConst().getNum(), 4}));
        diag.removeVertex(v);
    }
    ...
    void pivotPauli(ZXDiagram& diag, const Vertex v0, const Vertex v1) { // TODO: phases
    ...
    static void ensureInterior(ZXDiagram& diag, const Vertex v) {
        ...
        for (const auto& [to, type]: edges) {
            ...
            const auto boundaryEdgeType = type == zx::EdgeType::Simple ? zx::EdgeType::Hadamard : zx::EdgeType::Simple;
    ...
    bool checkAndFuseGadget(ZXDiagram& diag, const Vertex v) {
        ...
        if (diag.degree(id0) == 2) {
            const auto& [v0, v0Etype] = diag.incidentEdges(id0)[0].to == v ? diag.incidentEdges(id0)[1] : diag.incidentEdges(id0)[0];
