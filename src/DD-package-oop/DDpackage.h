...
namespace dd {
    ...
    struct Node {
	    ...
	    char ident, symm; // special matrices flags
    };
    ...
    class Package {
	    ...
	    NodePtr terminalNode{};        // pointer to terminal node
        Edge DDone{}, DDzero{};            // edges pointing to zero and one DD constants
	    ...
	    unsigned int activeNodeCount = 0;             // number of active nodes
	    ...
	    std::array<unsigned long, 2> nOps{};                     // operation counters
	    std::array<unsigned long, 2> CTlook{}, CThit{};      // counters for gathering compute table hit stats
	    ...
	    Edge multiply2(Edge x, Edge y, int var);
	    ...
	    Edge CTlookup(Edge a, Edge b, CTkind which);
	    void CTinsert(Edge a, Edge b, Edge r, CTkind which);
    ...
    public:
        ...
        // DD creation
        inline Edge makeTerminal(Complex w) { return { terminalNode, w }; }

	    Edge makeNonterminal(unsigned short v, Edge *edge, bool cached = false);

	    inline Edge makeNonterminal(unsigned short v, std::array<Edge, NEDGE> edge, bool cached = false) {
	    	return makeNonterminal(v, edge.data(), cached);
	    };
	    Edge makeZeroState(int n);
	    Edge makeIdent(int x, int y);
	    Edge makeGateDD(const Matrix2x2 mat, unsigned short n, const int *line);
	    ...
	    long double fidelity(Edge x, Edge y);
	    ...
	    // checks
	    inline bool isTerminal(const Edge e) const {
	    ...
	    void TTinsert(unsigned short n, unsigned short m, unsigned short t, const short line[], Edge e);
