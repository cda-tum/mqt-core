...
public:
  static void import(QuantumComputation& qc, const py::object& circ) {
    ...
    for (const auto pyinst : data) {
      ...
      emplaceOperation(qc, instruction, qargs, cargs, params, qubitMap,
                       clbitMap);
    }
...
protected:
  ...
  static void addOperation(QuantumComputation& qc, OpType type,
    ...
    if (std::all_of(parameters.cbegin(), parameters.cend(), [](const auto& p) {
      ...
      qc.emplace_back<StandardOperation>(qc.getNqubits(), controls, target,
                                         type, fpParams);
    } else {
      qc.emplace_back<SymbolicOperation>(qc.getNqubits(), controls, target,
                                         type, parameters);
